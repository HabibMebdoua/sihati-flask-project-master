...
TWILIO_ACCOUNT_SID = 'ACd13f6cee3b233584032651ad94bbb928'
TWILIO_AUTH_TOKEN = 'a1c28259b59af6f01215a1e74173c535'
TWILIO_PHONE_NUMBER = '+12603683098'
...
app.config['MAIL_USERNAME'] = 'mamadoheffaf@gmail.com'
app.config['MAIL_PASSWORD'] = 'tmgu tijx ahjd xhkg'
app.config['MAIL_DEFAULT_SENDER'] = 'mamadoheffaf@gmail.com'
...
# MySQL Configuration
app.config['MYSQL_HOST'] = 'sihati.mysql.pythonanywhere-services.com'
app.config['MYSQL_USER'] = 'sihati'
app.config['MYSQL_PASSWORD'] = 'sihatidb2025'
app.config['MYSQL_DB'] = 'sihati$default'
...
@app.route('/search-doctors-Si7atAi', methods=['GET', 'POST'])
...
def book_appointment(doctor_id):
    ...
    if request.method == 'POST':
        ...
        # Check if the appointment already exists for this patient
        cur = mysql.connection.cursor()
        query = """
        ...
        """
        cur.execute(query, (doctor_id, appointment_date_mysql, appointment_time_mysql))
        booked_slot = cur.fetchone()
        
        if booked_slot:
            # This time slot is already booked by another patient
            error_message = "Ce créneau est déjà réservé. Veuillez choisir un autre moment."
            return render_template('booking.html', doctor_data=doctor_data, date_error=error_message, name=name, profile=profile, user=user, url=url, dashboard=dashboard)
...
def payment(doctor_id):
    ...
    booking_fee_amount = 100.0
    total_fee = doctor_fee + booking_fee_amount 
    ...
    return render_template('payment.html', doctor=doctor_data,doctor_id = doctor_id, appointment_date = appointment_date, appointment_time = appointment_time,
                          doctor_fee = doctor_fee, booking_fee=booking_fee_amount,  total_fee=total_fee, name=name, profile=profile, user=user, url=url, dashboard=dashboard)
...
def patient_register():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        query = "INSERT INTO registered_patients (Name, Email, Phone,blood_group,Date_of_Birth, Password, Patient_ID) VALUES (%s, %s, %s,%s,%s, %s, %s)"
        cur.execute(query, (full_name, email, phone_no,blood_group,dob, password, patient_id))
        ...
        msg.body = f"Dear {full_name},\n\nThank you for registering with Si7atAi.\n\nYou can now log in to access all the services and features offered by Si7atAi.\n\nWe look forward to serving you!\n\nBest regards,\nThe Si7atAi Team"
        mail.send(msg)
...
def doctor_register():
    if request.method == 'POST':
        ...
        google_maps_url = request.form.get("google_maps_url", "")  # Use get() method with default empty string
        email = request.form['email']
        ...
        session['google_maps_url'] = google_maps_url
...
def verify(email):
    if request.method == 'POST':
        ...
        if verification_code == stored_verification_code:
            ...
            google_maps_url = session.get('google_maps_url', '')
            ...
            cur = mysql.connection.cursor()
            query = "INSERT INTO registered_doctors (Doctor_ID, Name, Gender, Clinic_Name, Clinic_Address, Google_Maps_URL, Email, Specialization, Phone, Password) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
            cur.execute(query, (doctor_id, name, gender, clinic_name, clinic_address, google_maps_url, email, specialization, phone, password))
            ...
            session.pop('google_maps_url',None)
...
@app.route('/delete-appointment/<string:appointment_id>', methods=['GET'])
def delete_appointment(appointment_id):
    if session.get('user_type') != 'doctor':
        session['error'] = "You are not authorized to delete appointments."
        return redirect(url_for('unauthorized'))
    ...
    try:
        ...
        email = session['email']
        cur = mysql.connection.cursor()
        cur.execute("SELECT Doctor_ID FROM registered_doctors WHERE Email = %s", (email,))
        doctor_id = cur.fetchone()[0]

        # Verify that the appointment belongs to this doctor
        cur.execute("SELECT Doctor_ID FROM appointments WHERE Appointment_ID = %s", (appointment_id,))
        appointment_doctor = cur.fetchone()
        
        if not appointment_doctor or appointment_doctor[0] != doctor_id:
            session['error'] = "You are not authorized to delete this appointment."
            return redirect(url_for('unauthorized'))
        ...
        session['success'] = "Appointment deleted successfully."
        return redirect(url_for('doctor_dashboard'))

    except Exception as e:
        session['error'] = f"An error occurred while deleting the appointment: {str(e)}"
        return redirect(url_for('doctor_dashboard'))
...
def admin_dashboard():
    if session['user_type'] == 'admin':
        ...
        cur = mysql.connection.cursor()
        query = """
            SELECT appointments.Appointment_ID, registered_doctors.Name AS Doctor_Name, registered_doctors.Profile_URL AS Doctor_Profile_URL, registered_doctors.Doctor_ID, registered_doctors.Specialization, registered_patients.Name AS Patient_Name, registered_patients.Profile_URL AS Patient_Profile_URL, registered_patients.Patient_ID, appointments.Appointment_Date, appointments.Appointment_Time
            FROM appointments
            JOIN registered_doctors ON appointments.Doctor_ID = registered_doctors.Doctor_ID
            JOIN registered_patients ON appointments.Patient_ID = registered_patients.Patient_ID
...
def contact_form():
    if request.method == 'POST':
        ...
        body = f"Name: {name}\nEmail: {email}\nPhone: {phone}\nServices: {services}\nMessage: {message}"
        recipients = ['mamadoheffaf@gmail.com']
...
def check_availability():
    ...
    try:
        ...
        """
        cur.execute(query, (doctor_id, date_mysql))
        booked_times = cur.fetchall()
        cur.close()
        
        # Format times for front-end
        booked_slots = []
        for time_tuple in booked_times:
...
@app.route('/view-patient/<string:patient_id>')
def view_patient(patient_id):
    if session.get('user_type') != 'doctor':
        session['error'] = "You are not authorized to view patient details."
        return redirect(url_for('unauthorized'))

    cur = mysql.connection.cursor()
    cur.execute("SELECT * FROM registered_patients WHERE Patient_ID = %s", (patient_id,))
    row = cur.fetchone()

    if not row:
        cur.close()
        session['error'] = "Patient not found."
        return redirect(url_for('doctor_dashboard'))
    ...
    return render_template('view-patient.html', patient_data=patient_data, name=name, profile=profile, user=user, url=url, dashboard=dashboard)

def send_appointment_reminder(appointment, patient_email, patient_phone):
    """Send reminder for an upcoming appointment via email and SMS"""
    try:
        ...
        # Send email reminder
        msg = Message('Appointment Reminder', recipients=[patient_email])
...
Si7atAi Team"""
        ...
        # Send SMS reminder
        sms_message = f"Reminder: Your appointment with Dr. {doctor_name} is scheduled for {appointment_date} at {appointment_time}. Please arrive 10 minutes early."
...
def check_upcoming_appointments():
    ...
    try:
        ...
        query = """
            SELECT a.*, d.Name as Doctor_Name, p.Email as Patient_Email, p.Phone as Patient_Phone
            FROM appointments a
            JOIN registered_doctors d ON a.Doctor_ID = d.Doctor_ID
            JOIN registered_patients p ON a.Patient_ID = p.Patient_ID
            WHERE a.Appointment_Date = %s
        """
        cur.execute(query, (tomorrow_date,))
        ...
        for appointment in appointments:
            appointment_dict = dict(zip(column_names, appointment))
            send_appointment_reminder(
                appointment_dict,
                appointment_dict['Patient_Email'],
                appointment_dict['Patient_Phone']
            )
...
# Schedule the appointment check to run daily at midnight
scheduler.add_job(check_upcoming_appointments, 'cron', hour=0, minute=0)
...
def admin_delete_doctor(doctor_id):
    if session.get('user_type') != 'admin':
        session['error'] = "You are not authorized to delete doctors."
        return redirect(url_for('unauthorized'))
    try:
        cur = mysql.connection.cursor()
        cur.execute("DELETE FROM registered_doctors WHERE Doctor_ID = %s", (doctor_id,))
        mysql.connection.commit()
        cur.close()
        session['success'] = "Doctor deleted successfully."
    except Exception as e:
        session['error'] = f"An error occurred while deleting the doctor: {str(e)}"
    return redirect(url_for('admin_dashboard'))

@app.route('/admin-delete-patient/<patient_id>', methods=['POST'])
def admin_delete_patient(patient_id):
    if session.get('user_type') != 'admin':
        session['error'] = "You are not authorized to delete patients."
        return redirect(url_for('unauthorized'))
    try:
        cur = mysql.connection.cursor()
        cur.execute("DELETE FROM registered_patients WHERE Patient_ID = %s", (patient_id,))
        mysql.connection.commit()
        cur.close()
        session['success'] = "Patient deleted successfully."
    except Exception as e:
        session['error'] = f"An error occurred while deleting the patient: {str(e)}"
    return redirect(url_for('admin_dashboard'))
